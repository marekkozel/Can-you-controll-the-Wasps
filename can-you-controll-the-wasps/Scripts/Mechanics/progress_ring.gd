...
# 产卵的按住进度、幼虫的饥饿倒计时都用它 / used by the hold progress and the hunger timer.
...
# 传多边形顶点（不用自己闭合），会预算好累计弧长 / pass corners, arc lengths are cached
...
func set_progress(t: float) -> void:
	...
	points = out if out.size() >= 2 else PackedVector2Array()  # Line2D 少于两点画不出来 / Line2D needs at least two points
