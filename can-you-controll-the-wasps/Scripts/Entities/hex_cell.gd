...
# 蜂巢的一格 / one hive cell.
# 这里只管状态和接线：建造进度、内容槽（空/卵/幼虫/腐烂）、把组件信号转成具体反馈。
# 效果本身在 JuiceComponent 和 ProgressRing 里，这里不实现。
#
# 按住这一格干什么取决于当前内容：空的 → 产卵，腐烂的 → 清理。
...
enum Content { NONE, EGG, LARVA, ROTTEN }
...
func _ready() -> void:
	# 显式接线。Visual 是被缩放/抖动的那层，Area2D 本身不能动
...
# Hive 调用。要在 add_child() 之后，不然 @onready 还没解析
...
func _on_larva_starved(_larva: Larva) -> void:
	content = Content.ROTTEN  # 尸体留在原地，等玩家按住清理
	_update_hold()
	_refresh_visual()
...
# 松手可能发生在格子外面，所以在这里收而不是 _on_input_event
...
func _refresh_visual() -> void:
	...
	_border.default_color = border_color.lerp(built_border_color, t)

	if not _juice.is_flashing():  # 闪白期间别抢颜色
